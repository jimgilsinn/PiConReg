		...
		if path == "/qrcode":
			...
			if query in data:
				...
				if query in reg_data:
					self.wfile.write("""
<html><head><meta name="description" content="duplicate"></head>
<body>User has already registered at the desk. Please check into this.</body></html>""")

				else:
					reg_write.write(query)
					reg_write.close()
					self.wfile.write("""
<html><head><meta name="description" content="registered"></head>
<body><BODY BGCOLOR="#66ff66">User has been registered successfully. Refreshing in 10 seconds.<meta HTTP-EQUIV="REFRESH" content="10; url=./"></body></html>""")

			# if it doesnt match then write user wasnt found
			else:
				self.wfile.write("""
<html><head><meta name="description" content="unknown"></head>
<body>[!] User was not found. Try manual methods :-(</body></html>""")
		...
		if self.path == "/":
			...
			self.end_headers()
			# title here
			self.wfile.write("<html><head><title>ConQR Central Registration Web Server</title></head>")
...
<p>Simply scan a QR code using a mobile device of some sort, doesn't matter which application. When scanned, they will be sent here to check to see if they have already registered or not. If they have, it will let you know and register them.
</p>
""")               
